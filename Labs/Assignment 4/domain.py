...
#         self.n = n
#         self.m = m
#         self.sensorIndex = [[None for _ in range(self.m)] for _ in range(self.n)]
#         self.sensorCount = 0
#         self.sensors = []
#         self.sensorPaths = []
...
#         self.sensor_path = [(self.drone_map.startingIndex, 0)]
#         self.battery_left = drone_map.battery
#         self.visited_sensors = [False for _ in range(self.drone_map.sensorCount)]
#         self.visited_sensors[self.drone_map.startingIndex] = True
#
#     def coverage(self):
#         pass
#
...
class DroneMap:
    ...
    def initialize_map(self, nr_of_sensors, wall_fill_factor=0.2):
        self.surface = [[2 if random.random() < wall_fill_factor else 0 for _ in range(self.m)] for _ in range(self.n)]
        while nr_of_sensors > 0:
            sx = random.randrange(self.n)
            sy = random.randrange(self.m)
            if self.surface[sx][sy] != 2:
                self.surface[sx][sy] = 1
                nr_of_sensors -= 1
            ...
            if not self.visited_sensors[sensor] and self.drone_map.paths_between_sensors[current_sensor][sensor] is not None:
                for spend_energy in range(1, MAX_SENSOR_COVERAGE + 1):
                    ...
                    if self.battery_left < distance_between_sensors + spend_energy:
                        continue
                    ponder = (1 / distance_between_sensors) ** beta * (tau ** alpha)
                    next_cell = [(sensor, spend_energy), ponder]
